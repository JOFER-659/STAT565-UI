main <- function(x,n){
  ...
  ## SANITY CHECKS (FIRST)
  ###########################
  if(length(x)<3){stop("k in k-allele model must be at least 3")}
  if(sum(x<0)!=0){stop("population relative frequencies cannot be negative")}
  if(sum(x)!=1){stop("population relative frequencies should sum to 1")}
  ...
  scripts_to_source <- c("sim.dirichlet.R", "sim.data.R", "calc.sumstats.R", "calc.suplik.R", "sim.par.R", "rej.R", "constants.R")
  ...
  stats = calc.sumstats(x)
  
  # Calculate supremum of the loglikelihood
  sup.lik <<- calc.suplik(stats) 
  # global variable
  
  # Sample the posterior distribution
  post.sample = rej(n, k, stats) 
